...
class _BookingsScreenState extends State<BookingsScreen> {
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Row(
          children: [
            ...
            SizedBox(width: 10),
            Text("Cancelar Reserva", style: TextStyle(fontWeight: FontWeight.bold)),
          ],
        ),
  ...
  Widget _buildBookingCard(BuildContext context, Map<String, dynamic> booking, String bookingId) {
    ...
    final String status = (booking['status'] ?? 'confirmado').toString().toLowerCase();
    
    bool estaCerca = status == 'confirmado' && diferencia.inMinutes <= 60 && diferencia.inMinutes > 0;
            ...
            children: [
              if (estaCerca)
                Container(
                  ...
                  color: esMuyCritico ? const Color(0xFFFFEBEE) : const Color(0xFFFFF3E0),
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Icon(Icons.access_alarm_rounded, size: 14, color: esMuyCritico ? Colors.red : Colors.orange[800]),
                      const SizedBox(width: 8),
                      Text(
                        esMuyCritico 
                          ? "¡APÚRATE! Faltan solo ${diferencia.inMinutes} min"
                          : "Tu viaje inicia en ${diferencia.inMinutes} min",
                        style: TextStyle(color: esMuyCritico ? Colors.red : Colors.orange[900], fontSize: 10, fontWeight: FontWeight.bold),
                      ),
                    ],
                  ),
                ),
                ...
                children: [
                  ListTile(
                    ...
                    title: Text("$origen → $destino", 
                      ...
                      style: TextStyle(
                        fontWeight: FontWeight.bold, fontSize: 14, color: const Color(0xFF1A4371),
                      )),
                    subtitle: Column(
                    ...
                    trailing: Column(
                      ...
                      children: [
                        Text(TripMateFormat.currencyCLP(booking['precio'] ?? 0), 
                          style: TextStyle(fontWeight: FontWeight.bold, color: esCancelado ? Colors.grey : const Color(0xFF2BB8D1))),
                        const Icon(Icons.chevron_right, size: 16, color: Colors.grey),
                      ],
                    ),
                  ),
                  Positioned(top: 0, right: 0, child: _buildStatusBadge(status, esPasado)),
                ],
              ),
  ...
  Widget _buildStatusBadge(String status, bool esPasado) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: bgColor, borderRadius: const BorderRadius.only(bottomLeft: Radius.circular(15))),
      child: Text(text, style: TextStyle(
        color: text == "CANCELADO" ? Colors.red : (text == "FINALIZADO" ? Colors.grey : const Color(0xFF1A4371)),
  ...
  Widget _buildConductorInfo(String? driverId) {
    return FutureBuilder<DocumentSnapshot>(
      ...
      builder: (context, snap) {
        if (!snap.hasData) return const CircleAvatar(radius: 18, child: Icon(Icons.person, size: 18));
        ...
        return Column(
          ...
          children: [
            ...
            Text(data['nombre']?.toString().split(' ')[0] ?? "Driver", style: const TextStyle(fontSize: 9, fontWeight: FontWeight.bold)),
          ],
        );
      },
    );
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            _detailItem(Icons.my_location, "Origen", _getText(booking['origen'])),
            _detailItem(Icons.location_on, "Destino", _getText(booking['destino'])),
            _detailItem(Icons.access_time, "Salida", _formatearFecha(booking['fechaSalida'])),
            _detailItem(Icons.paid_outlined, "Precio", TripMateFormat.currencyCLP(booking['precio'] ?? 0)),
